...
import { metaData, upload, uuidEntityParam } from "@app-validations/general.js";
import { Hono } from "hono";
import { z } from "zod";

const brand = new Hono<App>()
  .use(auth("admin"))
  .put(
    "/:entity/image",
  ...
  .get("/", acl("brand:list"), validator("query", metaData), async (c) => {
    return c.json(await withMeta(Brand, await c.req.valid("query")));
  });
